...
function limparConfigLocal(){try{Object.keys(CONFIG_PADRAO||{}).forEach(chave=>localStorage.removeItem(chave));}catch(e){}}
...
async function salvarConfigSupabase(chave,valor){
  ...
  setConfigSistema(chave,(data&&data.valor!==undefined)?data.valor:valor);
  limparConfigLocal();
  return true;
}
