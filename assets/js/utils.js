...
function setConfigSistema(chave,valor){window.configSistema=window.configSistema||{}; window.configSistema[chave]=valor; salvarJsonLocal(chave,valor);}
async function carregarConfiguracoesSupabase(){
  window.configSistema={...clonarConfig(CONFIG_PADRAO)};
  Object.keys(CONFIG_PADRAO).forEach(chave=>{const local=lerJsonLocal(chave,null); if(local!==null)window.configSistema[chave]=local;});
  try{
    const {data,error}=await buscarConfiguracoesLoja();
    if(error){console.warn('Configurações no Supabase indisponíveis:',error.message); return false;}
    (data||[]).forEach(row=>{window.configSistema[row.chave]=row.valor; salvarJsonLocal(row.chave,row.valor);});
    return true;
  }catch(e){console.warn('Falha ao carregar configurações do Supabase:',e); return false;}
}
async function salvarConfigSupabase(chave,valor){
  setConfigSistema(chave,valor);
  const {error}=await salvarConfigLojaBanco(chave,valor);
  if(error){alert('Não salvou no Supabase. Rode o SQL atualizado e confira permissões da tabela loja_config. Erro: '+error.message); throw error;}
  return true;
}
