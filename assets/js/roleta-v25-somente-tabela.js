/* V25 - ROLETA SOMENTE SUPABASE (SEM localStorage)
...
(function(){
  ...
  const VERSAO = 'Roleta V25 - SOMENTE TABELA SUPABASE';
  ...
  function atualizarPainelStatus(extra){
    let el = document.getElementById('status-roleta-supabase-v25');
    const box = document.getElementById('lista-premios-roleta');
    if(!el && box){
      el = document.createElement('div');
      el.id = 'status-roleta-supabase-v25';
  ...
  window.girarRoletaV25 = async function(){
  ...
  window.girarRoleta = window.girarRoletaV25;
